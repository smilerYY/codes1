...
if __name__ == '__main__':
    ...
    if args.resume is not None:
        ...
        if len(ckpt_files) != 0:
            ...
            writer = SummaryWriter(os.path.join(experiment_path, 'Loss_Accuracy'))
    ...
    for epoch in range(start_epoch, args.epochs+1):
        ...
        for iter, batch in enumerate(train_dataloader):
            ...
            if (iter + 1) % args.log_every == 0:
                ...
                writer.add_scalar('Loss/train', avg_loss, int((epoch-1)*total_steps+(iter+1)*args.batch_size))

        t = timer.t()
        prog = (epoch-start_epoch+1)/(args.epochs + 1 - start_epoch + 1)
        t_elapsed, t_all = utils.time_text(t), utils.time_text(t / prog)
        print('Times: {}/{}'.format(t_elapsed, t_all))
        if epoch % args.test_every == 0:
            ...
            for valid_dataloader in valid_dataloaders:
                ...
                writer.add_scalar('Accuracy/test/{}/psnr'.format(name), float(avg_psnr), int((epoch-1)*total_steps+(iter+1)*args.batch_size))
                #
                writer.add_scalar('Accuracy/test/{}/ssim'.format(name), float(avg_ssim), int((epoch-1)*total_steps+(iter+1)*args.batch_size))
